...
import {
    ...
    getPreferredTheme,
    getPrompts,
    removePrompt, setPreferredTheme,
    setPrompts,
    updatePrompt
        ...
        >
            <div className={colorScheme + "-theme"}>
                <div className="theme-switcher-buttons">
                    {isDarkTheme && <IconButton onClick={toggleColorScheme} name="sun" alt="Toggle color scheme"/>}
                    {!isDarkTheme && <IconButton onClick={toggleColorScheme} name="moon" alt="Toggle color scheme"/>}
                </div>
                {children}
            </div>
        </MantineProvider>
        ...
        <ThemeProvider>
            <main>
                ...
                {promptsState?.length > 0 &&
                    <PromptsAccordion data={promptsState} updatePrompt={update} removePrompt={remove}
                                      setPrompts={set}/>}
            </main>
        </ThemeProvider>
    )
