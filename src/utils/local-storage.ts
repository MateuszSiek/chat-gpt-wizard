...
export async function updateHistory(chatId: string, promptId: string): Promise<PromptHistory[]> {
    ...
    return localStorage.set({promptsHistory: newHistory}).then(({promptsHistory}) => promptsHistory);
}
...
export async function getPromptFromHistory(chatId: string): Promise<Prompt | undefined> {
